...
close_out:
    if (close(dev->fd_out) == 0)
        dev->fd_out = -1;
close_in:
    if (close(dev->fd_main) == 0)
        dev->fd_main = -1;
...
static void linux_close(struct wif *wi)
{
    ...
    if ((pl->fd_in > 0) && ((pl->drivertype == DT_BCM43XX) || (pl->drivertype == DT_IPW2200)))
    {
        if (close(pl->fd_in) == 0)
            pl->fd_in = -1;
    }
	if (pl->fd_out > 0)
	{
        if (close(pl->fd_out) == 0)
            pl->fd_out = -1;
	}
	if (pl->fd_main > 0)
	{
        if (close(pl->fd_main) == 0)
            pl->fd_main = -1;
	}
